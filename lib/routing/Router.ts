...
import { HTTPMethod, Route, RouteHandler, RouteHandlerInput, RouteHandlerResponse } from './model'
...
export class Router {
    ...
    constructor(lambdaEvent: ALBEvent|APIGatewayProxyEventV2) {
        const isALBEvent = (e: ALBEvent|APIGatewayProxyEventV2): e is ALBEvent => {
            return (e as ALBEvent).path !== undefined
        }

        const isAPIGatewayEvent = (e: ALBEvent|APIGatewayProxyEventV2): e is APIGatewayProxyEventV2 => {
        ...
        if (isALBEvent(lambdaEvent)) {
            ...
            this.queryStringParameters = (lambdaEvent.queryStringParameters || {}) as Record<string,string>
    ...
    registerRoute(route: Route): void {
        const _isRouteMatch = (): boolean => {
            ...
            if(route.pathPattern.includes(':')) {
                ...
                const regex = new RegExp('^'+pathPattern.replace(/:\w+/g, '([\\w\\d-\\.]+)') + '$')
                const regexMatch = this.path.match(regex)

                if(regexMatch === null) { return false; }
                
                const pathParameterKeys = (pathPattern.match(/\:\w+/g) || []).map((match) => match.substring(1))
                const pathParameterValues = regexMatch.slice(1)
                const pathParameters: Record<string,string> = pathParameterKeys.reduce((accumulator, key,idx) => {
                ...
                return route.httpMethod === this.httpMethod && route.pathPattern === this.path
            }
        }
    ...
    routeRequest(): Promise<RouteHandlerResponse> {
        ...
        if (!route) {
            return Promise.reject({statusCode: 404, body: {
